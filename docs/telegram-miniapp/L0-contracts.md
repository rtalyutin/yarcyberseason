...
Поправка desktop от 16.09.2026: `close()` закрывает Mini App через `Telegram.WebApp.close()`; в обычном браузере переводит на `/`, и UI подписывает это действие «На сайт». Закрытие не вызывает обработчик «Назад». В Telegram Esc закрывает приложение, если клавиша не обрабатывается полем ввода/выбором, IME или другим обработчиком; listener удаляется при `dispose()`.
