...
- Разрешение 19.09.2026: явное «Публикуй» после отчёта по Матчам разрешило выпуск подготовленных Формата, Расписания и Матчей на действующем домене, включая main. Выпуск выполнен и проверен ниже. Новые срезы шага 6, BotFather и сообщения этим разрешением не охватываются.
...
- Локальный browser preview ранее блокировался `ERR_BLOCKED_BY_CLIENT`; после разрешённого выпуска выполнен живой Chrome smoke на production: прямой вход в Матчи, переходы Формат → Расписание → Матчи, screenshot, viewport/scrollWidth 1363/1363 PASS. Реальное раскрытие карточек на production не проверено: опубликованных матчей нет. Клавиатурная/мобильная матрица и настоящий Telegram Android/iOS/Desktop — NOT_VERIFIED; Node/SSR их не заменяет.
- `operation_status` выпуска 19.09.2026: SUCCEEDED; deployment/exposure VERIFIED на действующем публичном домене. Main без force перемещён с `8862d0d19e90b2c7a260d4993fe43f8b18f0bdbd` на `33fb2a313462eb3027f4f0511ea6bac0b359d9d2`; последующая запись этого журнала меняет только документацию. Бот не менялся.
