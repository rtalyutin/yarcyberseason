...
## Приоритетная поправка — 16.09.2026, desktop/закрытие

Исправление desktop/закрытия опубликовано и проверено на действующем домене. Разрешение пользователя — «Публикуй»; main `8862d0d19e90b2c7a260d4993fe43f8b18f0bdbd`, дерево `bfe4f8e52b31dad53c616d88170f93cd9774c300` совпадает с проверенной версией `1e326f10e00bc384cce66cfe4d2802bffe1d892e`. 16.09.2026 в 12:41–12:42 UTC root и независимый verifier подтвердили точное совпадение 5/5 публичных HTML/JS/CSS с dist/client; корректные MIME, без SPA fallback вместо assets. Браузер на публичном домене подтвердил широкий интерфейс 1299 px при окне 1363 px, кнопку «На сайт», 16 участников в две колонки и возврат на главную miniapp. Build и 109/109 тестов PASS. Закрытие настоящего Telegram Desktop и отдельная мобильная регрессия новой версии остаются НЕ проверенными. Пользователь уточнил штатный процесс: обновление репозитория автоматически обновляет стенд; панель хостинга не требуется, запрашивать её из-за первого старого readback было ошибкой. Подробности — [desktop-fix.md](desktop-fix.md). Принятый шаг 5 сохраняется; следующий плановый — шаг 6.
