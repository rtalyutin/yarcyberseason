...
- `operation_status=PREPARED` для пакета и `NOT_STARTED` для внешней пробы, локальное `evidence_status=VERIFIED` только в перечисленных автоматических проверках; общий `gate_verdict=BLOCKED`, `action_decision=REQUEST_INPUT` для серверной конфигурации. `hypothesis_assessment=NOT_ASSESSED`.
...
- Следующее конкретное действие: выбрать доступный сервер с токеном, проверить числовой ID выбранного личного диалога и выполнить уже разрешённые два сообщения / одно редактирование по сценарию `rich-card.md`. Повторное разрешение на этот же тест не запрашивать; токен вводится на сервере, не в переписке. Шаг 7 не начат и не открыт. Существующая ежедневная задача остаётся на следующем незавершённом шаге 6; расписание и её настройки этой операцией не меняются.
