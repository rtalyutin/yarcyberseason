...
- Проверка доступности 20.09.2026: `YCS_TELEGRAM_BOT_TOKEN` в текущей среде не настроен; callable Telegram-коннектора нет. Значения секретов не читались и не выводились. Отправка не предпринималась.
