...
Исправление подготовлено по скриншоту Романа: desktop-композиция, отдельное закрытие, Esc и сохранение выхода при ошибке данных. Источник и ограничения проверки — [desktop-fix.md](desktop-fix.md). Пользователь разрешил публикацию словом «Публикуй» 16.09.2026. PR #5 объединён в main: `8862d0d19e90b2c7a260d4993fe43f8b18f0bdbd`; дерево `bfe4f8e52b31dad53c616d88170f93cd9774c300` совпадает с проверенным исправлением `1e326f10e00bc384cce66cfe4d2802bffe1d892e`. Build и 109/109 тестов PASS. Публикация на домене пока НЕ подтверждена: публичная проверка продолжает получать предыдущую сборку. Разрешение действует; повторно его не спрашивать. При продолжении сначала проверить публичную версию и штатный запуск обновления хостинга; успешный merge не выдавать за завершённый выпуск. Принятый шаг 5 не перезапускается; шаг 6 остаётся следующим плановым.
