...
- Ревизия `implementation-5`, 19.09.2026: продолжать реализацию по ТЗ 1.1 и дополнению §13 (1.2 от 17.09.2026), последовательно до шага 16. Это не прежнее поручение только подготовить документ.
...
- За этот запуск: ограниченная часть шага 6 — Формат и Расписание из общей модели, пустые состояния, локальные тесты и независимая проверка. Полный шаг 6 пока не закрывается.
...
- **Шаг 6/16 — IN_PROGRESS.** Реализованы Формат и Расписание. Код на GitHub `733288381445bdee400241cb4e616d7428958c77`; ветка `codex/telegram-step6-sections`, база main `8862d0d19e90b2c7a260d4993fe43f8b18f0bdbd`. Его tree `c3cbcb9f9fc6c73e2cb72267d57a666e0de9e2ad` точно совпадает с локальной проверенной ревизией `3dde1754b235ecbc0ad63c4262869dfbe5898927`.
...
- Технические проверки после исправлений независимого verifier: build PASS, `node --test tests/*.test.mjs` 120/120 PASS. Отчёт и ограничения: [step6-sections.md](step6-sections.md). Независимый `/root/verify_step6`: 63/63 выбранных теста и собственные held-out probes PASS на финальном code tree; [отчёт с первоначальными дефектами и повтором](independent-step6-sections.md). `evidence_status=VERIFIED`, `gate_verdict=PASS` только для описанного SSR/Node среза, не всего шага 6.
- Browser preview `http://127.0.0.1:4173/tg/tournament?section=schedule` заблокирован `ERR_BLOCKED_BY_CLIENT`. Свежий screenshot, визуальная приёмка, клавиатурное поведение в browser и настоящий Telegram Android/iOS/Desktop — NOT_VERIFIED. SSR их не заменяет.
...
- Следующий результат в том же шаге 6: Матчи с раскрываемыми подробностями, Swiss/плей-офф/итоги, затем version manifest и неблокирующее обновление по действию пользователя. Полная матрица и регрессия — шаг 7, разрешённый выпуск — шаг 8.
