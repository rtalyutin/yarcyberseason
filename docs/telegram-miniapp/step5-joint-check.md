...
До фактического HTTP/browser readback этот адрес является целью, а не подтверждённой рабочей ссылкой. Настройку Main Mini App и меню BotFather выполняет владелец по предоставленному после размещения URL. Проверка Telegram не выполнена.
