...
function Case({ item, round }) {
  ...
  return <section className={`fm-case fm-case-${item.teams}`}>
    ...
    {current ? <div key={round} className="fm-round"><div className="fm-round-title"><h3>Тур {round}</h3><span>{current.matches.length} матчей · осталось {current.remaining}</span></div><div className="fm-table-head"><span>Пара и результат после матча</span><span>Поражений до</span></div>
